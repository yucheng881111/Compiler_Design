...
void AstDumper::visit(VariableNode &p_variable) {
    ...
    std::printf("variable <line: %u, col: %u> %s %s\n",
                ...
                p_variable.getVarNamesCString(), p_variable.getVarTypeCString());
...
void AstDumper::visit(FunctionNode &p_function) {
    ...
    std::printf("function declaration <line: %u, col: %u> %s %s %s\n",
                ...
                p_function.getFuncNameCString(), p_function.getFuncTypeCString(),
                p_function.getArgList()
                );
...
void AstDumper::visit(CompoundStatementNode &p_compound_statement) {
    if(p_compound_statement.get_funcInvNode() != NULL){
        AstDumper::visit(*(p_compound_statement.get_funcInvNode()));
        return ;
    }
    
    if(p_compound_statement.get_retNode() != NULL){
        AstDumper::visit(*(p_compound_statement.get_retNode()));
        return ;
    }

    if(p_compound_statement.get_forNode() != NULL){
        AstDumper::visit(*(p_compound_statement.get_forNode()));
        return ;
    }

    if(p_compound_statement.get_whileNode() != NULL){
        AstDumper::visit(*(p_compound_statement.get_whileNode()));
        return ;
    }

    if(p_compound_statement.get_ifNode() != NULL){
        AstDumper::visit(*(p_compound_statement.get_ifNode()));
        return ;
    }

    if(p_compound_statement.get_printNode() != NULL){
        AstDumper::visit(*(p_compound_statement.get_printNode()));
        return ;
    }

    if(p_compound_statement.get_assNode() != NULL){
        AstDumper::visit(*(p_compound_statement.get_assNode()));
        return ;
    }

    if(p_compound_statement.get_readNode() != NULL){
        AstDumper::visit(*(p_compound_statement.get_readNode()));
        return ;
    }
...
void AstDumper::visit(FunctionInvocationNode &p_func_invocation) {
    ...
    std::printf("function invocation <line: %u, col: %u> %s\n",
                ...
                p_func_invocation.get_func_name());
...
void AstDumper::visit(ExpressionNode &p_expression) {
    p_expression.visitChildNodes(*this);
}
